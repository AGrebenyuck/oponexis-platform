...
import { mobileDeviceEnvironmentWhere } from './mobile-environment'
...
async function companionSmsDeviceText() {
	if (String(process.env.OPONEXIS_SMS_PROVIDER || '').trim().toLowerCase() !== 'companion') return null
	const device = await db.mobilePushDevice.findFirst({
		where: { enabled: true, smsPrimary: true, ...mobileDeviceEnvironmentWhere() },
		orderBy: { lastSeenAt: 'desc' },
		select: { label: true, manufacturer: true, model: true, phoneNumber: true, simSlot: true },
	})
	if (!device) return '⚠️ Brak aktywnego urządzenia Companion'
	const name = device.label || [device.manufacturer, device.model].filter(Boolean).join(' ') || 'Oponexis Companion'
	return `📱 ${name} · ${device.phoneNumber || 'numer SIM nieuzupełniony'} · SIM ${device.simSlot || 1}`
}
...
async function workOrderAttribution(leadId) {
	...
	return attributionFromLead(lead)
}
...
function workOrderText(order, extra = {}, title = '🔧 Nowe dane od klienta') {
	...
	lines.push(...leadSourceLines({
		source: extra.source || extra.customer?.source,
	...
	if (order.wantsInvoice) {
		...
		if (order.invoiceNip) lines.push(`   • NIP: <code>${html(order.invoiceNip)}</code>`)
		if (order.invoiceEmail) lines.push(`   • E-mail: <code>${html(order.invoiceEmail)}</code>`)
	}
...
export async function sendWorkOrderToTelegram(order, extra = {}) {
	...
	const [customer, attribution] = await Promise.all([
		customerSummary({ customerId: order.customerId, phone: order.phone }),
		workOrderAttribution(order.leadId),
	])

	const text = workOrderText(order, { ...extra, customer, attribution })
	const sent = await telegram('sendMessage', {
...
export async function updateWorkOrderMessage(order) {
	...
	return safeTelegram('editMessageText', {
		...
		text: withMessageIdFooter(
			workOrderText(order, { customer, attribution }, 'Dane klienta (zaktualizowane)'),
...
export async function updateSmsTrackerMessage() {
	...
	const deviceText = await companionSmsDeviceText()
	let text = ''
	if (!logs.length) {
		text = ['📲 SMS z formularzem', deviceText, '', 'Na razie brak wysłanych SMS z formularzem.'].filter(value => value !== null).join('\n')
	} else {
		const byDate = new Map()
		...
		const lines = ['📲 SMS z formularzem – od dziś i później', deviceText, ''].filter(value => value !== null)
		Array.from(byDate.keys())
			.sort()
			.forEach((key, index) => {
				const group = byDate.get(key) || []
...
async function handleSmsCommand(message) {
	...
	const deviceText = await companionSmsDeviceText()
	await reply(message.chat.id, ['Kliknij przycisk poniżej, aby otworzyć SMS.', deviceText].filter(Boolean).join('\n'), {
		reply_markup: {
			inline_keyboard: [
				[
					{
