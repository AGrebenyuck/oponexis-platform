...
export async function publishSmsContactEvent(event) {
	try {
		...
		const devices = await db.mobilePushDevice.findMany({
			where: { enabled: true, ...mobileDeviceEnvironmentWhere() },
			select: { id: true, installationId: true },
			take: 500,
		})
		if (!devices.length) return
		const result = await client.sendEachForMulticast({
			tokens: devices.map(device => device.installationId),
	...
	try {
		...
		const devices = await db.mobilePushDevice.findMany({
			where: dispatch.deviceId
				? { id: dispatch.deviceId, enabled: true, ...mobileDeviceEnvironmentWhere() }
				: { enabled: true, smsPrimary: true, ...mobileDeviceEnvironmentWhere() },
			select: { id: true, installationId: true },
			take: 500,
		})
		if (!devices.length) return
