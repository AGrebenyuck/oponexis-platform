...
import { getSmsGateMessageStatus, sendSmsGateMessage } from '@/lib/sms/smsGateClient'
import {
	bookingFormMessage,
...
function publicSiteUrl() {
	return (process.env.NEXT_PUBLIC_SITE_URL || process.env.PUBLIC_SITE_URL || 'https://oponexis.pl')
		.replace(/\/$/, '')
}
...
const GOOGLE_REVIEW_URL =
	...
	'https://g.page/r/CarQdmFTFAcbEAE/review'
...
export async function sendBookingFormSms({
	...
	try {
		...
		await createSmsContactEvent({
			direction: 'OUT',
			type: 'sms_booking_form_queued',
			phone,
			providerMessageId: result?.id || null,
			raw: { smsFormLogId: saved.id },
		})
...
export async function sendFormCompletedSms({ phone, name, visitDate, visitTime, workOrderId, profile }) {
	...
	const messageId = customMessageId('form-completed', String(workOrderId))
	let result
	try {
		result = await sendSmsGateMessage({
			phone,
			text,
			customId: messageId,
			profile,
		...
		if (error?.code !== 'sms_gateway_duplicate_request') throw error
		try {
			const existing = await getSmsGateMessageStatus(messageId, { profile })
			return { id: messageId, raw: existing.raw, duplicate: true }
		} catch {
			throw error
		}
	}
...
export async function sendReviewRequestSms({ phone, name, workOrderId, profile }) {
	const greeting = cleanCustomerName(name, phone)
	const text = `Dziękujemy za skorzystanie z usług Oponexis${greeting ? `, ${greeting}` : ''}. Jeśli wszystko przebiegło dobrze, będziemy wdzięczni za krótką opinię: ${GOOGLE_REVIEW_URL}`
	const messageId = customMessageId('review-request', String(workOrderId))
	let result
	try {
		result = await sendSmsGateMessage({ phone, text, customId: messageId, profile })
	} catch (error) {
		if (error?.code !== 'sms_gateway_duplicate_request') throw error
		const existing = await getSmsGateMessageStatus(messageId, { profile })
		return { id: messageId, raw: existing.raw, duplicate: true }
...
export async function sendAppointmentChangedSms({
	phone,
	name,
	previousVisit,
	nextVisit,
	...
	const text = `Dzień dobry${cleanCustomerName(name, phone) ? ` ${cleanCustomerName(name, phone)}` : ''}, termin wizyty w Oponexis został zmieniony: ${previousVisit} → ${nextVisit}.`
	const result = await sendSmsGateMessage({
		phone,
		text,
		customId: customMessageId('appointment-change', `${workOrderId}-${nextVisit.replace(/\D/g, '')}`),
		profile,
	})
	await createSmsContactEvent({
		direction: 'OUT',
		type: 'sms_appointment_changed',
		phone,
		message: text,
		providerMessageId: result?.id || null,
		raw: { workOrderId, previousVisit, nextVisit },
	})
	return result
}
...
export async function sendBookingFormReminder(log, profile) {
	...
	await createSmsContactEvent({
		direction: 'OUT',
		type: 'sms_booking_reminder_queued',
		phone: log.phone,
		providerMessageId: result?.id || null,
		raw: { smsFormLogId: log.id },
	})
