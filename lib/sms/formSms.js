...
async function customerContext({ leadId, phone, name }) {
	const lead = leadId
		? await db.lead.findUnique({
				where: { id: leadId },
				select: { id: true, name: true, customerId: true, firstTouchSource: true },
		  })
		: null
	...
	return {
		...
		leadId: lead?.id || null,
		savedDetails,
		sourceKnown: Boolean(lead?.firstTouchSource) || Boolean(customer?.source && customer.source !== 'Inne'),
	}
}
