...
export function smsGateConfigured(profile) {
	if (String(process.env.OPONEXIS_SMS_PROVIDER || '').trim().toLowerCase() === 'companion') {
		return Boolean(process.env.OPONEXIS_MOBILE_API_TOKEN?.trim())
	}
...
export async function sendSmsGateMessage({ phone, text, customId, profile }) {
	if (String(process.env.OPONEXIS_SMS_PROVIDER || '').trim().toLowerCase() === 'companion') {
		const { enqueueCompanionSms } = await import('@/lib/sms/companionSmsClient')
		return enqueueCompanionSms({ phone, text, customId, profile })
	}
	const {
		username,
		password,
...
export async function getSmsGateMessageStatus(messageId, { profile } = {}) {
	if (String(messageId || '').startsWith('cmp:')) {
		const { getCompanionSmsStatus } = await import('@/lib/sms/companionSmsClient')
		return getCompanionSmsStatus(messageId)
	}
	...
	const url = new URL(`/3rdparty/v1/messages/${encodeURIComponent(messageId)}`, baseUrl)
	const res = await fetch(url, {
		headers: {
			Authorization: authHeader(username, password),
		},
	...
	const json = await res.json().catch(() => null)
	if (res.status === 404) {
		const listUrl = new URL('/3rdparty/v1/messages', baseUrl)
		...
		const messages = Array.isArray(listJson) ? listJson : listJson?.items || listJson?.data || []
		const message = messages.find(item => String(item?.id || item?.messageId || '') === messageId)
		if (listResponse.ok && message) {
			return {
				raw: message,
				state: extractMessageState(message),
				reason: extractMessageReason(message),
			}
		}
	}
	if (!res.ok) {
...
export async function deleteSmsGateWebhook({ profile, id }) {
	if (!id) throw new Error('Brak ID webhooka SMSGate.')
	const { profile: activeProfile } = await smsGateRequest(
		`/3rdparty/v1/webhooks/${encodeURIComponent(id)}`,
