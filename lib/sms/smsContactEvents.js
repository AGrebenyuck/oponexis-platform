...
import { publishSmsContactEvent } from '@/lib/mobile-push'
...
export async function createSmsContactEvent({
	...
	const created = await db.smsContactEvent.create({
		data: {
	...
	await publishSmsContactEvent(created)
	return created
}
...
export async function recordSmsDeliveryEvent({ event, payload, raw }) {
	...
	return createSmsContactEvent({
		...
		phone: payload.recipient || payload.to || payload.phoneNumber,
