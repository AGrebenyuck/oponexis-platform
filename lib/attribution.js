...
export function canonicalSourceFromAttribution(attribution) {
	...
	if (attribution?.gclid || attribution?.wbraid || attribution?.gbraid || source.includes('google') && /(cpc|ppc|paid|ads?)/.test(medium)) {
		return 'Google Ads'
	}
...
export function firstTouchLeadData(attribution) {
	...
	return {
		...
		firstTouchWbraid: attribution.wbraid,
		firstTouchGbraid: attribution.gbraid,
